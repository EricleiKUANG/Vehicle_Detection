    ...
    with tf.control_dependencies([shape_assert]):
      ...
      with slim.arg_scope(
        ...
        with tf.variable_scope(
         ...
         while blocknum < 5:
             temp = image_features['block'+str(blocknum)]
             temp = slim.conv2d(
                temp,
                num_outputs = depth,
                kernel_size = [1,1],
          ...
          temp = tf.image.resize_images(temp, tf.shape(second_last_feature_map)[1:3])

#if self._first_stage_features_outblock == 4 else ['block2', 'block3']
