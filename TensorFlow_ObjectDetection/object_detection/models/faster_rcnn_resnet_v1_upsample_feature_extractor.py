...
class FasterRCNNResnetV1UpsampleFeatureExtractor(
  ...
  def _filter_features(self, image_features):
    ...
    for key, feature in image_features.items():
      ...
      if feature_name in ['block1', 'block2', 'block3']:
    ...
    with tf.control_dependencies([shape_assert]):
      ...
      with slim.arg_scope(
        ...
        with tf.variable_scope(
          ...
          last_feature_map = image_features['block3']
          last_feature_map = slim.conv2d(
              ...
              padding='SAME',
              scope='block3_side')
          top_down = tf.image.resize_images(last_feature_map, tf.shape(image_features['block2'])[1:3])
